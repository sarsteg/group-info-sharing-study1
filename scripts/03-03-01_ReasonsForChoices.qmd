...
endorsement by condition, included in the paper.
...
apa_tbl <- desc_tbl %>%
  mutate(
    ...
    condition = factor(condition, levels = cond_order),
    variable  = factor(variable,  levels = var_order),
    # mean as percentage, no SD
    cell      = sprintf("%.0f%%", M * 100)
  ) %>%
  dplyr::select(variable, condition, cell) %>%
  tidyr::pivot_wider(names_from = condition, values_from = cell) %>%
  arrange(variable) %>%
  dplyr::select(variable, any_of(cond_order))

apa_tbl
```

## create new variable to count across categories

```{r}
REASONS_category <- filtered_data
```

```{r}
# cooperation
REASONS_category <- REASONS_category %>%
  mutate(
    REASONS_coOp = if_else(
      if_any(all_of(c("REASONS_1", "REASONS_2", "REASONS_3")), ~ .x == 1),
      1,
      0
    )
  )
...
REASONS_category <- REASONS_category %>%
  mutate(
    REASONS_coMp = if_else(
      if_any(all_of(c("REASONS_4", "REASONS_5", "REASONS_6", "REASONS_7", "REASONS_8")), ~ .x == 1),
      1,
      0
    )
  )
...
```{r}
REASONS_category <- REASONS_category %>% 
  select("REASONS_1", "REASONS_2", "REASONS_3", REASONS_coOp, "REASONS_4", "REASONS_5", "REASONS_6", "REASONS_7", "REASONS_8", REASONS_coMp, structure, motivation)
```

```{r}
names(REASONS_category)
# View(REASONS_category)
```

```{r}
endorse_freq <- REASONS_category %>%
  dplyr::select(REASONS_coOp, REASONS_coMp) %>%
  pivot_longer(
    cols      = everything(),
    names_to  = "Item",
    values_to = "Endorsed"
...
```{r}
endorse_freq <- REASONS_category %>%
  select(structure, motivation, REASONS_coOp, REASONS_coMp) %>%
  pivot_longer(
    cols      = c(REASONS_coOp, REASONS_coMp),
    names_to  = "Item",
    values_to = "Endorsed"
  ) %>%
  group_by(structure, motivation, Item) %>%   # keep Item in the group
  summarise(
    n_endorsed    = sum(Endorsed == 1, na.rm = TRUE),
    prop_endorsed = mean(Endorsed == 1, na.rm = TRUE),
    .groups = "drop"
...
```{r}
var_order <- c("REASONS_coOp", "REASONS_coMp")
...
apa_tbl <- endorse_freq %>%
  mutate(
    # combine structure and motivation into one condition label
    condition = paste(structure, motivation, sep = "–"),
    condition = factor(condition, levels = cond_order),

    # use Item as the row variable
    variable  = factor(Item, levels = var_order),

    # percentage from prop_endorsed, no decimals
    cell      = sprintf("%.0f%%", prop_endorsed * 100)
  ) %>%
  dplyr::select(variable, condition, cell) %>%
  tidyr::pivot_wider(
