...
```{r}
confint(state_model, level = 0.95)
```
...
reg_table <- tidy(
  ...
  mutate(
    Predictor = case_when(
      ...
      term == "DirtyDozen_Machiavellianism" ~ "Machiavellianism",
      term == "structure_number01" ~ "Structure",
      term == "motivation_number01" ~ "Motivation",
      term == "structure_number01:motivation_number01" ~
...
sheet_name <- "Cooperation Results"
...
saveWorkbook(
  wb,
  "../tables/cooperation_regression.xlsx",
  overwrite = TRUE
)
```
...
filtered_data %>%
  group_by(structure) %>%
  summarise(
    mean_value = mean(Manipulation_Mean, na.rm = TRUE),
    sd_value = sd(Manipulation_Mean, na.rm = TRUE)
  )
```
